...
def run_polopt_agent(env_fn, 
    ...
    # Restore from checkpoint if provided (for resuming training)
    if restore_path is not None:
        logger.log('Restoring policy weights from: %s' % restore_path, color='green')
        ckpt_path = os.path.join(restore_path, 'variables', 'variables')
        try:
            restore_vars = get_vars('pi') + get_vars('vf') + get_vars('vc') + get_vars('penalty')
            restore_saver = tf.train.Saver(var_list=restore_vars)
            restore_saver.restore(sess, ckpt_path)
        except Exception:
            logger.log('Penalty vars not found in checkpoint, restoring pi/vf/vc only.', color='yellow')
            restore_vars = get_vars('pi') + get_vars('vf') + get_vars('vc')
            restore_saver = tf.train.Saver(var_list=restore_vars)
            restore_saver.restore(sess, ckpt_path)
        logger.log('Policy weights restored successfully.', color='green')
    ...
    cur_penalty = 0
    cum_cost = float(initial_cumulative_cost)
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        if (local_epoch % save_freq == 0) or (epoch == (start_epoch + epochs - 1)):
