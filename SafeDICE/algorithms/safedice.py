...
class SafeDICE(tf.keras.layers.Layer):
    ...
    ## MODIFIED TO HANDLE CASES WHERE OPTIMIZER STATE IS NOT SAVED (E.G. WHEN LOADING ONLY NETWORK WEIGHTS)
    def set_training_state(self, training_state):
        def _assign_values(variables, params):
            if len(variables) != len(params):
                print(f"⚠️  Warning: Variable count mismatch. Expected {len(variables)}, got {len(params)}")
                print("This is normal when loading only network weights without optimizer state.")
                return  # Skip assignment if mismatch
            
            for variable, (name, value) in zip(variables, params):
                if variable.name != name:
                    print(f"⚠️  Warning: Variable name mismatch. Expected {variable.name}, got {name}")
                    continue
                variable.assign(value)

        # Load network weights (always needed)
        _assign_values(self.cost.variables, training_state['cost_params'])
        _assign_values(self.critic.variables, training_state['critic_params'])
        _assign_values(self.actor.variables, training_state['actor_params'])
        
        # Load optimizer state (optional - may not match if creating new optimizers)
        if 'cost_optimizer_state' in training_state and len(training_state['cost_optimizer_state']) > 0:
            try:
                _assign_values(self.cost_optimizer.variables(), training_state['cost_optimizer_state'])
            except:
                print("⚠️  Skipping cost optimizer state loading")
        
        if 'critic_optimizer_state' in training_state and len(training_state['critic_optimizer_state']) > 0:
            try:
                _assign_values(self.critic_optimizer.variables(), training_state['critic_optimizer_state'])
            except:
                print("⚠️  Skipping critic optimizer state loading")
        
        if 'actor_optimizer_state' in training_state and len(training_state['actor_optimizer_state']) > 0:
            try:
                _assign_values(self.actor_optimizer.variables(), training_state['actor_optimizer_state'])
            except:
                print("⚠️  Skipping actor optimizer state loading")
