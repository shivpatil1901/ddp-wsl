...
vec_env = gym.vector.AsyncVectorEnv([lambda: gym.make(env_name)] * 2)
...
# checkpoint dir
checkpoint_dir = './weights_LR_HC'
os.makedirs(checkpoint_dir, exist_ok=True)
timestamp = time.strftime("%Y%m%d_%H%M%S")
checkpoint_filepath = f"{checkpoint_dir}/{algorithm}_{robot_name}{task_name}_seed{seed}_{timestamp}.pickle"

# Setup CSV logging
csv_log_dir = './logs_LR_HC'
os.makedirs(csv_log_dir, exist_ok=True)
csv_filepath = f"{csv_log_dir}/{algorithm}_{robot_name}{task_name}_seed{seed}_{timestamp}.csv"
csv_file = open(csv_filepath, 'w', newline='')
csv_writer = csv.writer(csv_file)
# Write CSV header
csv_writer.writerow([
    'iteration', 'elapsed_time', 'cost_loss', 'nu_loss', 'actor_loss',
    'anti_expert_nu', 'unlabeled_nu', 'init_nu', 'unlabeled_adv', 'anti_expert_adv',
    'unlabeled_weight', 'anti_expert_weight', 'unlabeled_min_weight', 'unlabeled_max_weight',
    'anti_expert_min_weight', 'anti_expert_max_weight', 'unlabeled_reward', 'anti_expert_reward',
    'alpha', 'eval_return', 'eval_cost', 'CVar50', 'CVar20', 'CVar10', 'CVar5'
])
csv_file.flush()
print(f"CSV logging to: {csv_filepath}")
...
# Track saved checkpoints to keep only last 2
saved_checkpoints = []
    ...
    while training_info['iteration'] < config['total_iterations']:
        ...
        if training_info['iteration'] % config['log_interval'] == 0:
            ...
            csv_row = [
                ...
                info_dict.get('cost_loss', np.nan),
                info_dict.get('nu_loss', np.nan),
                info_dict.get('actor_loss', np.nan),
                info_dict.get('anti_expert_nu', np.nan),
                info_dict.get('unlabeled_nu', np.nan),
                info_dict.get('init_nu', np.nan),
                info_dict.get('unlabeled_adv', np.nan),
                info_dict.get('anti_expert_adv', np.nan),
                info_dict.get('unlabeled_weight', np.nan),
                info_dict.get('anti_expert_weight', np.nan),
                info_dict.get('unlabeled_min_weight', np.nan),
                info_dict.get('unlabeled_max_weight', np.nan),
                info_dict.get('anti_expert_min_weight', np.nan),
                info_dict.get('anti_expert_max_weight', np.nan),
                info_dict.get('unlabeled_reward', np.nan),
                info_dict.get('anti_expert_reward', np.nan),
                info_dict.get('alpha', np.nan),
                info_dict.get('eval', np.nan),
                info_dict.get('cost', np.nan),
                info_dict.get('CVar50', np.nan),
                info_dict.get('CVar20', np.nan),
                info_dict.get('CVar10', np.nan),
                info_dict.get('CVar5', np.nan)
            ]
        ...
        if training_info['iteration'] % config['save_interval'] == 0 and training_info['iteration'] > 0:
            checkpoint_filepath_iter = checkpoint_filepath.replace('.pickle', f'_iter{training_info["iteration"]}.pickle')
            imitator.save(checkpoint_filepath_iter, training_info)
            
            # Keep only last 2 checkpoints
            saved_checkpoints.append(checkpoint_filepath_iter)
...
# Close CSV file
csv_file.close()
print(f"Training complete. Logs saved to: {csv_filepath}")
